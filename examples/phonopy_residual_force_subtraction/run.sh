...
ALLEGRO_MODEL=${ALLEGRO_MODEL:-/mnt/d/lin_temp/codex/allegro/Allegro-OAM-L-0.1.nequip.pth}
...
ALLEGRO_MODEL=$(resolve_path "${ALLEGRO_MODEL}")
if [[ ! -f "${ALLEGRO_MODEL}" ]]; then
  echo "Allegro model not found: ${ALLEGRO_MODEL}" >&2
  echo "Set ALLEGRO_MODEL=/path/to/model.pth or edit BCAR before running." >&2
  exit 1
fi
...
sed -i "s|^MODEL=.*|MODEL=${ALLEGRO_MODEL}|" "${WORK_DIR}/BCAR"
...
(
  ...
  phonopy --fz perfect/vasprun.xml displacements/disp-*/vasprun.xml > phonopy_fz.log 2>&1
)

rm -rf output
mkdir -p output/perfect output/displacements

for file in SPOSCAR phonopy_disp.yaml FORCE_SETS phonopy_displacements.log phonopy_fz.log; do
...
for file in POSCAR CONTCAR OUTCAR OSZICAR vasprun.xml vpmdk.log; do
  if [[ -f "${WORK_DIR}/perfect/${file}" ]]; then
    cp "${WORK_DIR}/perfect/${file}" "output/perfect/${file}"
  fi
done
...
echo "phonopy_residual_force_subtraction finished: output/ updated"
