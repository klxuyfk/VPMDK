...
Import the synchronous, standard-library-only client without loading the
calculation runtime:
...
`socket_path` follows the CLI resolution order: explicit value,
`VPMDK_SOCKET`, then the per-user default.

- `run(workdir=".", *, timeout=0.0, log_callback=None, event_callback=None)`
  submits one calculation and returns its successful terminal event.
- `status(*, timeout=2.0)` returns the server status object.
- `stop(*, force=False, timeout=60.0)` requests shutdown and returns the
  accepted shutdown event.

Positive timeouts cover the complete request exchange. A timed-out client does
not cancel work already accepted by the server. Methods are synchronous and do
not start a server or fall back to one-shot execution.

Client exceptions derive from `VPMDKClientError`:

- `ServerConnectionError` and `ProtocolError`
- `ClientTimeoutError`
- `RemoteCalculationError`, including `RemoteInputError` and
  `RemoteBackendMismatch`

Use [Server Mode](../user-guide/server-mode.md) for lifecycle, configuration,
queueing, and security behavior. The low-level server class and protocol helpers
are implementation-oriented; the CLI and `VPMDKClient` are the supported
