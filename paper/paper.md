...
A second problem is file-level compatibility. Many relaxation, molecular-dynamics, transition-state, phonon, charge-analysis, and structure-search workflows in computational materials science are organized around VASP-style directories and downstream tools that inspect `OUTCAR`, `OSZICAR`, `vasprun.xml`, `XDATCAR`, numbered image directories, or `CHGCAR`-like volumetric files. Researchers may want to replace the force model with an MLP while preserving those workflow contracts.  addresses this need by combining backend normalization with a VASP-style command-line surface, so established VASP-oriented automation can be reused while the underlying calculator is selected through `BCAR` or the Python API.
